...
(function () {
  ...
  const K_IST = 4.25;      // истинный коэффициент датчика, Па/мВ (в натурном опыте вышло 4,3)
  const U0 = 709.0;        // нуль датчика, мВ
  const G = 9.81;
  const M_MAX = 70;        // набор гирь, как в учебном примере, г
  ...
  function addMass(dm) {
    if (m + dm > M_MAX) {
      hint('Больше гирь в наборе нет: максимум ' + M_MAX + ' г (как в учебном примере).', true);
      return;
    }
  ...
  function render() {
    ...
    if (loaded.length >= 3) {
      ...
      html += `<p class="small">Ваша тарировка: <b>K = ${VL.fm(kR, 2)} Па/мВ</b> ` +
        `(в натурном опыте вышло 4,3 Па/мВ — расхождение ${VL.fm(dev, 1)} %). ` +
  ...
  function reset() {
    ...
    hint('Виртуальные данные очищены — на странице действуют учебный пример. Снимите нулевой отсчёт, чтобы начать заново.');
  }
