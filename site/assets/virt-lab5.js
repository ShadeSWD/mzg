...
(function () {
  ...
  const RHO = 1.2, Q = 354;                 // скоростной напор, Па (как в учебном примере)
  const VINF = Math.sqrt(2 * Q / RHO);      // 24,3 м/с
  /* дренажные точки: x мм, координаты на схеме, «истинный» Cp модели
   * (исправная носовая точка: Cp≈+1 в критической точке) и исходный Cp */
  const PTS = [
  ...
  function reset() {
    rows.clear(); render();
    $('v5hint').textContent = 'Виртуальные данные очищены — на странице действуют учебный пример.';
  }
  ...
  function render() {
    ...
    if (rows.has(0)) {
      ...
      html += `<div class="note tip"><b>Носовая точка у вас исправна:</b> C<sub>p</sub>(0) = ${VL.fm(cp0, 2)} — ` +
        'близко к +1, как и положено в критической точке. В натурном опыте она показала лишь 0,014: ' +
        'дренажное отверстие модели, судя по всему, было засорено.</div>';
    }

    html += '<h3>Ваша эпюра C<sub>p</sub>(x)</h3><div id="v5chart"></div>' +
      '<p class="small">Серым пунктиром — эпюра натурного опыта, ' +
